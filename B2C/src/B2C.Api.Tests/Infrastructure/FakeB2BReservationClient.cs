...
namespace B2C.Api.Tests.Infrastructure
{
    public sealed class FakeB2BReservationClient : IB2BReservationClient
    {
        ...
        public void Reset()
        {
            ReserveBehavior = ReserveBehavior.AlwaysSucceed;
            UnreserveAlwaysFails = false;
            FulfillAlwaysFails = false;
            ReserveCalls.Clear();
            UnreserveCalls.Clear();
            FulfillCalls.Clear();
            _reserveResults.Clear();
        }
            ...
            Guid idempotencyKey, IReadOnlyList<ReserveLine> items, CancellationToken ct)
        {
            ...
            var result = ReserveBehavior switch
            {
                ...
                ReserveBehavior.AlwaysFail => new ReserveResult(false, BuildFailed(items)),
        ...
        public Task<bool> UnreserveAsync(Guid idempotencyKey, IReadOnlyList<ReserveLine> items, CancellationToken ct)
        {
            UnreserveCalls.Add(idempotencyKey);
            return Task.FromResult(!UnreserveAlwaysFails);
        }

        public Task<bool> FulfillAsync(Guid orderId, IReadOnlyList<ReserveLine> items, CancellationToken ct)
        {
            FulfillCalls.Add(orderId);
            return Task.FromResult(!FulfillAlwaysFails);
        }

        private static IReadOnlyList<ReserveFailedItem> BuildFailed(IReadOnlyList<ReserveLine> items)
        {
            var failed = new List<ReserveFailedItem>(items.Count);
            foreach (var i in items)
                failed.Add(new ReserveFailedItem(i.SkuId, i.Quantity, 0, ReserveFailReason.OutOfStock));
            return failed;
        }

    }

    public enum ReserveBehavior { AlwaysSucceed, AlwaysFail }
